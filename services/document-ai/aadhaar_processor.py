        ...
        if pytesseract is not None:
            try:
                ...
                if digits_match:
                    ...
                    # Targeted Aadhaar Number Strip Extraction (Above Red Line)
                    try:
                        strip = gray[int(th * 0.71):int(th * 0.89), int(tw * 0.16):int(tw * 0.88)]
                        strip_ocr = pytesseract.image_to_string(strip, config='--oem 3 --psm 7 -c tessedit_char_whitelist=0123456789 ')
                        clean_strip = re.sub(r'[^0-9]', '', strip_ocr)
                        if len(clean_strip) >= 12:
                            detected_aadhaar_digits = f"{clean_strip[:4]} {clean_strip[4:8]} {clean_strip[8:12]}"
                    except Exception:
                        pass

                # Check QR code decoded data for 12-digit UID
                if not detected_aadhaar_digits and val:
                    uid_m = re.search(r'uid="(\d{12})"', val) or re.search(r'\b\d{12}\b', val)
                    if uid_m:
                        raw_uid = uid_m.group(1) if 'uid=' in uid_m.group(0) else uid_m.group(0)
                        detected_aadhaar_digits = f"{raw_uid[:4]} {raw_uid[4:8]} {raw_uid[8:]}"
